    ...
    public class AppsPublicController : Controller
    {
                                                        ...
                                                        string posData = null, CancellationToken cancellationToken = default)
        {
            ...
            if (app == null)
                return NotFound();
            var settings = app.GetSettings<PointOfSaleSettings>();
            if (string.IsNullOrEmpty(choiceKey) && !settings.ShowCustomAmount && settings.DefaultView != "cart")
            {
                return RedirectToAction(nameof(ViewPointOfSale), new { appId = appId });
            }
            string title = null;
            ...
            {
                if (!settings.ShowCustomAmount && settings.DefaultView != "cart")
                    return NotFound();
                price = amount;
                title = settings.Title;
